...
const Exercises: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Process data from the view - translations are already included
  ...
  const getExerciseName = (translations: any) => {
    ...
    const ro = translations.ro || translations['ro'];
    return en?.name || ro?.name || 'Unknown Exercise';
  };
      ...
      <nav className="container pt-4" aria-label="Fitness navigation">
        <NavigationMenu>
          <NavigationMenuList>
            ...
            <NavigationMenuItem>
              <NavLink to="/dashboard?cat=health&sub=configure" className={({ isActive }) => `${navigationMenuTriggerStyle()} ${isActive ? 'bg-accent/50' : ''}`}>
                Configure
              </NavLink>
            </NavigationMenuItem>
          </NavigationMenuList>
