...
export default function WeightRepsForm({ exercise, onLogged }: Props) {
  const setIndex = (exercise?.sets?.filter((s: any) => s.is_completed).length ?? 0) + 1;
  const [weight, setWeight] = useState<number>(exercise?.target_weight_kg ?? 20);
  const [reps, setReps] = useState<number>(exercise?.target_reps ?? 10);
