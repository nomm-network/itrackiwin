-- Latest readiness from BOTH sources (JSON answers + readiness_logs)
CREATE OR REPLACE VIEW public.v_latest_readiness AS
WITH src AS (
  -- From pre_workout_checkins.answers (JSONB)
  SELECT
    user_id,
    created_at,
  ...
  -- From readiness_logs (operator's table)
  SELECT
    user_id,
    created_at,
    energy::numeric,
    ...
    illness,
    alcohol,
    COALESCE(supplements, '[]'::jsonb) AS supplements
...
-- Fire it automatically when workout_exercises are created
...
FOR EACH ROW
EXECUTE FUNCTION public.initialize_warmup_for_exercise(NEW.id);