-- Step 4: Equipment Intelligence - Create tables first
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'gym_equipment_overrides_equipment_slug_fkey'
  ) THEN
    ALTER TABLE public.gym_equipment_overrides 
    ADD CONSTRAINT gym_equipment_overrides_equipment_slug_fkey 
    FOREIGN KEY (equipment_slug) REFERENCES public.equipment_defaults(slug);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'exercise_equipment_profiles_equipment_slug_fkey'
  ) THEN
    ALTER TABLE public.exercise_equipment_profiles 
    ADD CONSTRAINT exercise_equipment_profiles_equipment_slug_fkey 
    FOREIGN KEY (equipment_slug) REFERENCES public.equipment_defaults(slug);
  END IF;
END $$;
