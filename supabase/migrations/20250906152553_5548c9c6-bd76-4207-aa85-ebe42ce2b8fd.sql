-- Step 4: Equipment Intelligence - defaults + gym overrides + micro-plates
...
CREATE TABLE IF NOT EXISTS public.gym_equipment_overrides (
  ...
  default_slug text NOT NULL REFERENCES public.equipment_defaults(slug),
...
CREATE INDEX IF NOT EXISTS idx_eep_slug ON public.exercise_equipment_profiles(default_slug);
...
LEFT JOIN public.gym_equipment_overrides geo ON geo.default_slug = ed.slug;
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  FROM target t
  JOIN public.v_effective_equipment ve ON ve.slug = t.default_slug
