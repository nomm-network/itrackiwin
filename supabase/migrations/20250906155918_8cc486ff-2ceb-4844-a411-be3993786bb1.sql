-- Step 6: Commissions UX + Marketplace prep
...
-- c) Mentor catalog (by category/tags)
...
GROUP BY mp.id, mp.display_name, mp.slug, mp.status;

-- d) Local mentors (mentors with active memberships in a city)
...
-- RLS policies for marketplace views
CREATE POLICY "v_marketplace_gyms_read" ON public.gyms
FOR SELECT USING (status = 'active');

CREATE POLICY "v_marketplace_mentors_read" ON public.mentor_profiles
FOR SELECT USING (status = 'active');