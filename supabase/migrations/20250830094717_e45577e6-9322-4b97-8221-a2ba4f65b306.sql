-- Clear existing exercises and insert properly with new foreign key structure
DELETE FROM exercises CASCADE;

-- Insert exercises with correct foreign key references
...
exercise_inserts AS (
  ...
  FROM (VALUES 
    ('barbell-bench-press', 'Barbell Bench Press', 'horizontal_push', 'bench-press', 'dual_load', 'medium', 'olympic-barbell', 'mid_chest', 'chest', 4, true, 20, 'per_side', 100, ARRAY['compound', 'strength'], ARRAY[(SELECT id FROM muscle_groups_lookup WHERE slug = 'shoulders'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'triceps')]),
    ('overhead-press', 'Overhead Press', 'vertical_push', 'overhead-press', 'dual_load', 'medium', 'olympic-barbell', 'front_delts', 'arms', 5, true, 20, 'per_side', 90, ARRAY['compound', 'strength'], ARRAY[(SELECT id FROM muscle_groups_lookup WHERE slug = 'triceps'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'traps')]),
    ('triceps-pushdown', 'Triceps Pushdown', 'isolation', 'pushdown', 'stack', 'low', 'cable-machine', 'triceps_lateral_head', 'arms', 2, false, NULL, 'total', 80, ARRAY['isolation', 'accessories'], ARRAY[]::uuid[]),
  ...
  LEFT JOIN movement_patterns_lookup mp ON mp.slug = ed.movement_pattern_slug
  LEFT JOIN movements_lookup mv ON mv.slug = ed.movement_slug
