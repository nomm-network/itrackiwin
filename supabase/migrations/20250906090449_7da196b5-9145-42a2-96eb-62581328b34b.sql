-- Step 1g: Complete setup with explicit table references in RLS

-- Create mentor_clients table (if not exists)
...
-- Create RLS policies for mentors with explicit table references
CREATE POLICY mentors_owner_rw ON public.mentors
  FOR SELECT 
  TO authenticated
  USING (auth.uid() = mentors.user_id);

CREATE POLICY mentors_owner_mod ON public.mentors
  FOR INSERT 
  TO authenticated
  WITH CHECK (auth.uid() = mentors.user_id);
...
CREATE POLICY mentors_public_read ON public.mentors
  ...
  USING (mentors.is_active = true AND mentors.accepting_clients = true);

-- Create RLS policies for mentor_clients with explicit table references
...
CREATE POLICY mentor_clients_client_read ON public.mentor_clients
  FOR SELECT 
  TO authenticated
  USING (mentor_clients.user_id = auth.uid());
