-- BLOCKER 3: Fix targets/warmup first-render issue
-- Enhance start_workout to compute proper base loads and warmup
...
BEGIN
  ...
  -- Get a readiness score (simplified for now)
  SELECT COALESCE(AVG(
    CASE 
      WHEN energy IS NOT NULL AND sleep IS NOT NULL THEN (energy + sleep) / 2.0
      ELSE 70.0 
    END
  ...
  LOOP
    ...
    -- Update the workout exercise with computed values
    ...
    SET 
      ...
      target_origin = CASE 
        ...
        ELSE 'last_workout'
      END,
      readiness_adjusted_from = v_base_weight::text,
