-- 2.1. Bar catalog (seed with proper type casting)
insert into bar_types (name, default_weight, unit) 
select name, weight, unit::weight_unit from (
  values
    ('Olympic Bar 20 kg', 20, 'kg'),
    ('Women''s Bar 15 kg', 15, 'kg'),
    ('Technique Bar 7 kg', 7, 'kg'),
    ('Trap/Hex Bar 25 kg', 25, 'kg')
) as v(name, weight, unit)
where not exists (
  select 1 from bar_types bt where bt.name = v.name
);
