-- =====================================================
-- WORKOUT CATALOG CLEANUP: SCHEMA MIGRATION (Step 2)
-- =====================================================
...
-- 3) Populate slug from EN translation (fallback: first translation)
WITH en AS (
  SELECT e.id, et.name
  FROM exercises e
  LEFT JOIN exercises_translations et
    ON et.exercise_id = e.id AND et.language_code = 'en'
),
fallback AS (
  SELECT DISTINCT ON (e.id) e.id, et.name
  FROM exercises e
  LEFT JOIN exercises_translations et ON et.exercise_id = e.id
  ORDER BY e.id, et.language_code
)
UPDATE exercises e
SET slug = slugify(COALESCE(en.name, fallback.name))
FROM en
LEFT JOIN fallback ON fallback.id = e.id
WHERE e.id = en.id AND (e.slug IS NULL OR e.slug = '');
