...
BEGIN
  ...
  WHERE t.typname = 'load_type'
    AND e.enumlabel = 'bodyweight';

  IF NOT FOUND THEN
    ALTER TYPE load_type ADD VALUE 'bodyweight';
  END IF;
END$$;
...
SELECT
  ...
  3,     -- complexity_score
  'stack_pin'::text AS loading_hint,
...
SELECT
  ...
  'bodyweight'::load_type AS load_type,  -- default (can add plate in workout)
  ...
  false, -- is_unilateral
  2,     -- complexity_score
  'bodyweight'::text AS loading_hint,
  80,    -- popularity_rank
