...
-- Admin view (read-only list for UI)
create or replace view public.v_admin_users_overview as
select
  u.id as user_id,
  coalesce(u.display_name, u.full_name, 'User') as name,
  u.email,
  u.created_at,
  coalesce(jsonb_agg(
...
left join public.mentor_category_assignments mca
  on mca.mentor_user_id = u.id and mca.is_active
group by u.id, u.display_name, u.full_name, u.email, u.created_at;
...
language sql stable security definer set search_path=public as $$
  select lc.id, lc.slug, coalesce(lct.name, lc.slug) as name
  from public.life_categories lc
  left join public.life_categories_translations lct
    on lct.life_category_id = lc.id and lct.language_code = 'en'
  order by name;
$$;