...
BEGIN
  ...
  IF p_template_id IS NOT NULL THEN
    -- Own or accessible template
    IF NOT EXISTS (
      SELECT 1
      FROM public.workout_templates wt
      WHERE wt.id = p_template_id
            AND (wt.user_id = v_user_id OR COALESCE(wt.is_public, false) = true)
    ) THEN
    ...
    INSERT INTO public.workout_exercises (
      ...
      target_sets,
      target_reps,
      target_weight_kg,      -- ✅ workout_exercises only stores kg
      weight_unit,           -- kept for UI display if your table has it
      rest_seconds,
      notes
    )
    SELECT
      ...
      te.default_sets,
      te.target_reps,
      -- ✅ Use kg if present, otherwise convert legacy target_weight by unit
      COALESCE(
        te.target_weight_kg,
        CASE
          WHEN te.target_weight IS NULL THEN NULL
      ...
      -- Keep whatever unit was on the template; you render in that unit
