...
-- Add grip_ids, display_name, and grip_key to workout_exercises
ALTER TABLE public.workout_exercises
  ADD COLUMN grip_ids uuid[] NULL,
  ADD COLUMN display_name text NULL,
  ADD COLUMN grip_key text NULL;

-- Create function to update grip_key
CREATE OR REPLACE FUNCTION public.update_grip_key()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
  -- Update grip_key when grip_ids changes
  IF NEW.grip_ids IS NULL THEN
    NEW.grip_key := NULL;
  ELSE
    NEW.grip_key := array_to_string(array(SELECT unnest(NEW.grip_ids::text[]) ORDER BY 1), ',');
  END IF;
  RETURN NEW;
END;
$function$;

-- Create trigger to automatically update grip_key
CREATE TRIGGER tr_update_grip_key
  BEFORE INSERT OR UPDATE ON public.workout_exercises
  FOR EACH ROW
  EXECUTE FUNCTION public.update_grip_key();
...
-- Add new unique constraint that includes grip_key
ALTER TABLE public.personal_records 
