-- Phase 1: Database Cleanup & Foundation (Fixed)
...
-- Create RLS policies for user_exercise_estimates (with conditional creation)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'user_exercise_estimates' 
    AND policyname = 'Users can manage their own exercise estimates'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_user_exercise_estimates_updated_at'
  ) THEN
...
-- Seed some common handles if they don't exist
