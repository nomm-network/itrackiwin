-- Step 5: Analytics & Ops - KPI Views and Commission System (Final)

-- 1) Gym activity (members, coaches, sessions)
CREATE OR REPLACE VIEW public.v_gym_activity AS
SELECT
  g.id as gym_id,
  g.name,
  COUNT(DISTINCT ugm.user_id) as active_members,
  COUNT(DISTINCT gcm.mentor_id) FILTER (WHERE gcm.status='active') as active_coaches,
...
JOIN public.user_gym_memberships ugm ON ugm.user_id = w.user_id
...
-- 6) Commission summary (month and last month) - Fixed to join through agreement
