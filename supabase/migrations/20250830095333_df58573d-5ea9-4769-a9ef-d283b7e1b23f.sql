-- Update the exercise insertion script to use movement slug instead of name with correct enum types
-- Clear existing exercises and insert properly with movement slugs
DELETE FROM exercises CASCADE;

-- Insert exercises with correct slugs
...
exercise_inserts AS (
  ...
  FROM (VALUES 
    ('barbell-bench-press', 'Barbell Bench Press', 'horizontal_push', 'bench_press', 'dual_load', 'medium', 'olympic-barbell', 'mid_chest', 'chest', 4, true, 20, 'per_side', 100, ARRAY['compound', 'strength'], ARRAY[(SELECT id FROM muscle_groups_lookup WHERE slug = 'shoulders'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'triceps')]),
    ('overhead-press', 'Overhead Press', 'vertical_push', 'overhead_press', 'dual_load', 'medium', 'olympic-barbell', 'front_delts', 'arms', 5, true, 20, 'per_side', 90, ARRAY['compound', 'strength'], ARRAY[(SELECT id FROM muscle_groups_lookup WHERE slug = 'triceps'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'traps')]),
    ('triceps-pushdown', 'Triceps Pushdown', 'isolation', 'tricep_extension', 'stack', 'low', 'cable-machine', 'triceps_lateral_head', 'arms', 2, false, NULL, 'total', 80, ARRAY['isolation', 'accessories'], ARRAY[]::uuid[]),
    ...
    ('deadlift', 'Deadlift', 'hinge', 'deadlift', 'dual_load', 'high', 'olympic-barbell', 'erector_spinae', 'legs', 7, true, 20, 'per_side', 98, ARRAY['compound', 'strength'], ARRAY[(SELECT id FROM muscle_groups_lookup WHERE slug = 'glutes'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'hamstrings'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'traps'), (SELECT id FROM muscle_groups_lookup WHERE slug = 'forearms')])
  ...
  WHERE eq.id IS NOT NULL AND mg.id IS NOT NULL AND bp.id IS NOT NULL AND mv.id IS NOT NULL AND mp.id IS NOT NULL
  RETURNING id, slug
)
-- Insert translations
