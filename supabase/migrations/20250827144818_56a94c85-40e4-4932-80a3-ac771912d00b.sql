-- Exercise seeding with foreign key constraint handling
-- Uses system UUID with temporary constraint disabling

BEGIN;

-- Temporarily disable foreign key constraint for system user seeding
...
handles AS (SELECT id FROM handles WHERE slug = ANY(ARRAY['straight-bar'])),
grips AS (SELECT id FROM grips WHERE slug = ANY(ARRAY['overhand-wide','overhand'])),
ins_ex AS (
...
ins_grips AS (
  ...
  SELECT ex.id, g.id, ROW_NUMBER() OVER ()
...
ins_ex AS (
  ...
  SELECT p.ex_slug, p.system_user, bp.id, eq.id, p.movement_pattern, p.load_type, true, true
  FROM params p, bp, eq
  ON CONFLICT (slug) DO UPDATE SET
    body_part_id = EXCLUDED.body_part_id,
    equipment_id = EXCLUDED.equipment_id,
...
ins_ex AS (
  ...
  SELECT p.ex_slug, p.system_user, bp.id, eq.id, p.movement_pattern, p.load_type, false, true
  FROM params p, bp, eq
  ON CONFLICT (slug) DO UPDATE SET
    body_part_id = EXCLUDED.body_part_id,
    equipment_id = EXCLUDED.equipment_id,
...
-- Re-enable foreign key constraints and RLS
