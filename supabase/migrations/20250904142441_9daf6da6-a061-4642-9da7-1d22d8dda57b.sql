-- Step 1: Backfill template targets (safe, idempotent)
-- This fills template_exercises.target_weight_kg using the best available source per user+exercise:
-- • last non-warmup set in the last 60 days,
-- • user estimate (user_exercise_estimates.estimated_weight) – any of ('working','rm10','estimated_working'),
-- • PR × 0.70 (if you keep a PR view; the query tolerates if it doesn't exist),
-- • finally a conservative 20 kg default.
...
with candidates as (
  select
    ...
    -- 1) last non-warmup working set within 60 days
    (
      ...
      where w.user_id = wt.user_id
        ...
        and coalesce(ws.set_type::text, 'normal') <> 'warmup'
      order by coalesce(w.ended_at, w.started_at) desc, ws.created_at desc
      limit 1
    )                                     as last_working_kg,
