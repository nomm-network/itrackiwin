-- Fix start_workout function - correct column name mapping
...
DECLARE
  v_workout_id uuid;
  v_user_id    uuid;
  v_cols_workout_ex   text[] := ARRAY['target_sets','target_reps','target_weight','weight_unit','rest_seconds','notes','grip_ids'];
...
BEGIN
  ...
  /*
    ...
    Optional pairs we'll try to map:
      ...
      workout_exercises.target_weight      <- template_exercises.target_weight_kg
  ...
  */
  ...
  v_select_cols := format('%L, te.exercise_id, COALESCE(te.order_index, rn)', v_workout_id);
  ...
  -- Compose dynamic INSERT … SELECT with safer ORDER BY
  v_sql := format($q$
    WITH te_rows AS (
      SELECT te.*,
             ROW_NUMBER() OVER (ORDER BY COALESCE(te.order_index, 999999), te.id) AS rn
