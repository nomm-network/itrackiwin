-- =========================================================
-- BRO COACH UPGRADE - COMBINED MIGRATION
-- =========================================================
-- Why: unify gym equipment, simplify experience level, add
-- user muscle priorities, prepare translations, and keep
-- personal gym support.
-- =========================================================
...
-- 3) Temporary mapping from old table -> enum value
-- We map by slug (examples: 'new-to-fitness','returning','regular','very-experienced')
WITH m AS (
  SELECT
    upf.id AS upf_id,
...
-- 4) Make the new enum required and drop old column
ALTER TABLE public.user_profile_fitness
...
-- Drop old experience_level text column
DO $$
BEGIN
  IF EXISTS(
    SELECT 1 FROM information_schema.columns
...
-- 5) Replace experience_level_params table with an enum-keyed config
...
VALUES
  ...
  ('regular', 0.6, 0.8, 1, 2, 150, 180, 0.1, true),
  ('very_experienced', 0.7, 0.9, 1, 2, 180, 240, 0.05, true)
ON CONFLICT (experience_level) DO NOTHING;

-- 7) Rename enum column to final name (cleaner)
ALTER TABLE public.user_profile_fitness
...
CREATE TABLE IF NOT EXISTS public.gym_machines (
  ...
  label text,                           -- e.g., "Lat Pulldown #2 near mirrors"
  stack_values numeric[] DEFAULT NULL,  -- e.g., {36,41,46,51,57} kg
  aux_increment numeric DEFAULT NULL,   -- e.g., 1 or 2 kg magnetic add-on
...
-- 2) Popularity/usage signals (for auto-populating public gyms)
...
-- 3) Gym Admins (owners/managers with edit rights)
...
-- (Optional) simple view that picks the "popular machines" for auto populate
...
-- ---------------------------------------------------------
-- Add gym_machine_id (nullable) + CHECK: exactly one of the two is set.
DO $$
BEGIN
  IF NOT EXISTS(
    SELECT 1 FROM information_schema.columns
...
-- Add constraint to ensure XOR semantics (one or the other, not both)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ---------------------------------------------------------
-- E) UNIFIED VIEW FOR BRO: available equipment for a user
-- ---------------------------------------------------------
-- This merges shared gym machines (if the user has a membership)
-- and personal user gym machines (fallback/personal setups).
...
VALUES
  ...
  ('enum.experience_level.regular', 'en', 'Regular exerciser'),
...
-- Add updated_at trigger for experience_level_configs
