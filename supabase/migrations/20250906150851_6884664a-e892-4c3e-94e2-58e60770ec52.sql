-- Step 3: Coach-Gym Workflow + QR Onboarding + City Directory (Fixed Views)
...
-- Create unique index for coach-client-gym combination
CREATE UNIQUE INDEX IF NOT EXISTS idx_coach_client_links_unique 
ON public.coach_client_links (coach_user_id, client_user_id, COALESCE(gym_id, '00000000-0000-0000-0000-000000000000'::uuid));
...
BEGIN
  ...
  ELSIF r.kind = 'coach_to_gym' THEN
    -- Use the existing gym_coach_memberships table structure
...
-- Views for Directory & Coach Ops (Fixed to match actual table structure)
