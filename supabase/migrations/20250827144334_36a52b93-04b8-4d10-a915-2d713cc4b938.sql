-- Exercise seeding following canonical pattern
...
WITH params AS (
  SELECT
    ...
    'push'::movement_pattern            AS movement_pattern,
    'dual_load'::load_type              AS load_type,
    'bar'::load_medium                  AS load_medium,
...
ins_ex AS (
  INSERT INTO exercises (
    ...
    movement_pattern, load_type, load_medium,
    is_bar_loaded, is_public
  )
  SELECT
    p.ex_slug, p.system_user, bp.id, eq.id,
    p.movement_pattern, p.load_type, p.load_medium,
    true, true
  FROM params p, bp, eq
...
WITH params AS (
  SELECT
    ...
    'pull'::movement_pattern            AS movement_pattern,
    'stack'::load_type                  AS load_type,
    'stack'::load_medium                AS load_medium,
    '00000000-0000-0000-0000-000000000000'::uuid AS system_user
),
bp AS (
  SELECT id FROM body_parts b
