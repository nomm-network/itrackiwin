-- Fix the upsert function completely to remove all ambiguous references
-- Also ensure mood column exists and add the comprehensive solution
...
-- 3) Simple read helper
CREATE OR REPLACE FUNCTION public.get_today_readiness()
RETURNS numeric
LANGUAGE sql
STABLE
AS $$
  ...
  WHERE rc.user_id = auth.uid()
    AND rc.checkin_date = (now() AT TIME ZONE 'UTC')::date
  LIMIT 1
$$;
...
BEGIN
  ...
  DROP POLICY IF EXISTS rc_user_write_own ON public.readiness_checkins;
  DROP POLICY IF EXISTS rc_user_update_own ON public.readiness_checkins;
  DROP POLICY IF EXISTS rc_user_read_own ON public.readiness_checkins;

  -- Create new policies
  CREATE POLICY rc_user_write_own
    ON public.readiness_checkins
    FOR INSERT TO authenticated
    WITH CHECK (user_id = auth.uid());

  CREATE POLICY rc_user_update_own
    ON public.readiness_checkins
    FOR UPDATE TO authenticated
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

  CREATE POLICY rc_user_read_own
    ON public.readiness_checkins
