...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'gym_admins_pkey' 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'gym_equipment_min_weight_check'
  ) THEN
    ALTER TABLE public.gym_equipment ADD CONSTRAINT gym_equipment_min_weight_check 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'gym_equipment_max_weight_check'
  ) THEN
    ALTER TABLE public.gym_equipment ADD CONSTRAINT gym_equipment_max_weight_check 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'gym_equipment_increment_check'
  ) THEN
    ALTER TABLE public.gym_equipment ADD CONSTRAINT gym_equipment_increment_check 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'gym_equipment_count_check'
  ) THEN
    ALTER TABLE public.gym_equipment ADD CONSTRAINT gym_equipment_count_check 
      CHECK (count >= 0);
  END IF;
END $$;
