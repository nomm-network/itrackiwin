-- Step 5: Analytics & Ops - KPI Views and Commission System
...
SELECT
  ...
  COUNT(DISTINCT w.id) FILTER (WHERE w.started_at >= now() - interval '7 days') as workouts_7d,
  COUNT(DISTINCT w.id) FILTER (WHERE w.started_at >= now() - interval '30 days') as workouts_30d
FROM public.gyms g
...
LEFT JOIN public.workouts w ON w.gym_id=g.id
...
GROUP BY w.gym_id, we.exercise_id, e.display_name;
