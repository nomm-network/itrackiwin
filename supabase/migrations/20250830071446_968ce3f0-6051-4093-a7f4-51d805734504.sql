-- Fix exercises insert - primary_muscle_id should reference muscles table, not muscle_groups
WITH equipment_lookup AS (
  SELECT id, slug FROM equipment
),
muscle_lookup AS (
  SELECT id, slug FROM muscles  -- Changed from muscle_groups to muscles
),
exercise_inserts AS (
  ...
  SELECT 
    ...
    eq.id,
    mg.id,  -- This now correctly references muscles.id
    ed.popularity_rank,
    true,
    NULL
  FROM (VALUES 
    ('barbell-bench-press', 'Barbell Bench Press', 'horizontal_push', 'dual_load', 'medium', 'olympic-barbell', 'pectoralis-major', 100),
    ('overhead-press', 'Overhead Press', 'vertical_push', 'dual_load', 'medium', 'olympic-barbell', 'deltoid-anterior', 90),
    ('triceps-pushdown', 'Triceps Pushdown', 'isolation', 'stack', 'low', 'cable-machine', 'triceps-brachii', 80),
    ('squat', 'Squat', 'squat', 'dual_load', 'high', 'olympic-barbell', 'quadriceps-femoris', 95),
    ('deadlift', 'Deadlift', 'hinge', 'dual_load', 'high', 'olympic-barbell', 'erector-spinae', 98)
  ...
  WHERE eq.id IS NOT NULL AND mg.id IS NOT NULL
