-- Create the missing unique constraints for readiness_checkins
-- First, let's see what constraints we need and create them

-- For per-workout uniqueness (one readiness per user per workout)
CREATE UNIQUE INDEX IF NOT EXISTS ux_readiness_user_workout 
ON public.readiness_checkins (user_id, workout_id) 
WHERE workout_id IS NOT NULL;

-- For per-day uniqueness (one readiness per user per day when no workout)
...
-- Now let's update the RPC function to use proper conflict resolution
...
BEGIN
  ...
  IF workout_id IS NOT NULL THEN
    ...
    ON CONFLICT (user_id, workout_id) WHERE workout_id IS NOT NULL
    DO UPDATE SET
  ...
  ELSE
    ...
    ON CONFLICT (user_id, checkin_date) WHERE workout_id IS NULL
    DO UPDATE SET
      energy=EXCLUDED.energy, sleep_quality=EXCLUDED.sleep_quality,
      sleep_hours=EXCLUDED.sleep_hours, soreness=EXCLUDED.soreness,
      stress=EXCLUDED.stress, mood=EXCLUDED.mood,
