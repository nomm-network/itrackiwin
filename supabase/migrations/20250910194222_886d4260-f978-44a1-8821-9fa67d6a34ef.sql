-- Fix the RPC function to use the correct weight column
...
BEGIN
  ...
  -- Get exercises with sets - fixed to use correct weight column
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', we.id,
    ...
    SELECT 
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'set_index', set_index,
          'set_kind', set_kind,
          'weight', COALESCE(weight_kg, weight), -- Try weight_kg first, fallback to weight
