-- Step 10: Rollout & Migration Hardening
...
-- 2. Compatibility view for weight resolution
CREATE OR REPLACE VIEW v_weight_resolution_active AS
SELECT
  COALESCE(g.id, 'default'::uuid) as gym_id,
  COALESCE(pp.unit, 'kg') as unit,
  COALESCE(pp.min_increment_kg, 1.25) as min_increment_kg,
  COALESCE(pp.min_increment_lb, 2.5) as min_increment_lb,
  COALESCE(pp.name, 'Global Default') as source_name,
  CASE 
    WHEN g.id IS NOT NULL AND pp.id IS NOT NULL THEN 'gym_profile'
    WHEN pp.is_default THEN 'global_fallback'
    ELSE 'system_default'
  END as resolution_source
FROM gyms g
LEFT JOIN plate_profiles pp ON pp.is_default = true -- Simplified for now
UNION ALL
...
UNION ALL
-- Add fixed bars as "racks"
SELECT
  NULL::uuid as gym_id, -- Global for now
  'fixed_bar' as kind,
  'kg' as unit,
  7.5 as min_weight,
  25 as max_weight,
...
-- 5. Enhanced resolution function with feature flag
...
BEGIN
  ...
  IF v_exercise_load_type IN ('dual_load') THEN
    v_best_total := closest_barbell_weight_kg(
      desired_kg, 
      20, -- Bar weight - could come from config
