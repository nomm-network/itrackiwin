-- Create comprehensive recalibration RPC function
...
DECLARE
  ...
  v_equipment_available boolean := true;
...
BEGIN
  ...
  WITH recent_sessions AS (
    ...
    LIMIT p_lookback_sessions * 3 -- More sets to analyze
  )
  SELECT 
    ...
    MAX(rs.weight) FILTER (WHERE rs.session_rank = 1), -- Most recent top weight
  ...
  -- Calculate consistency score (how consistent are the performances)
  IF array_length(v_rpes, 1) >= 2 THEN
    SELECT AVG(rpe_val) - STDDEV(rpe_val) INTO v_consistency_score
    FROM unnest(v_rpes) AS rpe_val;
  ELSE
    v_consistency_score := 7.0; -- Default moderate
  END IF;

  -- Determine progression factor based on feels and RPE
  IF array_length(v_feels, 1) >= 2 THEN
    ...
    DECLARE
      ...
      feel text;
    BEGIN
      FOREACH feel IN ARRAY v_feels[1:LEAST(3, array_length(v_feels, 1))] LOOP
        CASE feel
          WHEN '++', '+' THEN easy_count := easy_count + 1;
          WHEN '--', '-' THEN hard_count := hard_count + 1;
          ELSE NULL;
        END CASE;
      END LOOP;

      -- Progression logic
      IF easy_count >= 2 AND hard_count = 0 THEN
      ...
      ELSIF hard_count >= 2 OR (array_length(v_rpes, 1) > 0 AND AVG(rpe_val) > 8.5 FROM unnest(v_rpes) rpe_val) THEN
        v_progression_factor := 0.95 - (hard_count * 0.025); -- 5-10% deload
  ...
  IF v_last_top_weight IS NOT NULL THEN
    v_suggested_weight := ROUND((v_last_top_weight * v_progression_factor) / 2.5) * 2.5; -- Round to 2.5kg
  ELSE
    -- No history, use conservative starting weight based on experience
    v_suggested_weight := 40 + (v_muscle_priority * 10); -- Basic starting point
    v_notes := array_append(v_notes, 'No history - using conservative starting weight');
  END IF;

  -- Determine rep range based on muscle priority and experience
  v_suggested_reps := CASE 
    WHEN v_muscle_priority >= 4 THEN 8 -- Higher priority = moderate reps
    WHEN v_muscle_priority <= 2 THEN 12 -- Lower priority = higher reps
    ELSE 10 -- Default
  END;
