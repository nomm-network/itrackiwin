-- FIXED EXERCISE SEED SCRIPT
-- Corrected table aliases in CTE
...
-- 3) Resolve FK IDs for each row - FIXED ALIAS
resolved AS (
  SELECT
    ...
    mu.id      AS primary_muscle_id,  -- FIXED: was 'm.id'
    eq.id      AS equipment_id,       -- FIXED: was 'e.id'
    hg.id      AS handle_id,          -- FIXED: was 'h.id'
