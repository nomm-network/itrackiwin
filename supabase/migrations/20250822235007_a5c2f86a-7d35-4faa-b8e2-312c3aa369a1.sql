-- =========================================================
-- BRO COACH UPGRADE - PART 2 (after enum value added)
-- =========================================================

-- ---------------------------------------------------------
-- A) EXPERIENCE LEVEL: MIGRATE DATA + CONFIG TABLE
-- ---------------------------------------------------------
...
-- 2) Map existing data to enum values
WITH m AS (
  SELECT
    upf.id AS upf_id,
    CASE LOWER(COALESCE(upf.experience_level, 'new'))
      ...
      WHEN 'regular' THEN 'intermediate'
...
-- 3) Make the new enum required and drop old columns
ALTER TABLE public.user_profile_fitness
...
-- Insert config values for all enum values
...
-- 5) Rename enum column to final name
ALTER TABLE public.user_profile_fitness
