-- Phase 2: Business Logic Functions for Workout Assistant
  ...
  IF p_template_id IS NOT NULL THEN
    ...
    LOOP
      ...
      IF rec.default_sets IS NOT NULL AND rec.default_sets > 0 THEN
        ...
        SELECT v_we_id, s, 'working'::public.set_type, false
...
BEGIN
  ...
  v_base_rest := CASE v_set_kind
    WHEN 'warmup' THEN 30
    WHEN 'working' THEN 180  -- 3 minutes for working sets
    WHEN 'dropset' THEN 120
    WHEN 'failure' THEN 240  -- Longer rest after failure sets
    ELSE 120
  END;
...
WHERE is_completed = true AND set_kind = 'working';
