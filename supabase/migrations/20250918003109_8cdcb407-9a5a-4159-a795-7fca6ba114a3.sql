-- Fix the program_creator enum value in generate_ai_program function
CREATE OR REPLACE FUNCTION public.generate_ai_program(p_goal text, p_experience_level text, p_training_days_per_week integer, p_location_type text, p_available_equipment text[], p_priority_muscle_groups text[], p_time_per_session_min integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Map goal to enum value - FIX THE MAPPINGS
  v_goal_enum := CASE p_goal
    ...
    WHEN 'recomp' THEN 'recomp'::program_goal
    WHEN 'endurance' THEN 'general_fitness'::program_goal  -- Map endurance to general_fitness
    WHEN 'general_fitness' THEN 'general_fitness'::program_goal
    ELSE 'muscle_gain'::program_goal
  END;

  -- Map experience level to enum value - FIX THE MAPPINGS
  v_exp_enum := CASE p_experience_level
    WHEN 'new' THEN 'new'::experience_level
    WHEN 'returning' THEN 'returning'::experience_level
    WHEN 'intermediate' THEN 'intermediate'::experience_level
    WHEN 'advanced' THEN 'advanced'::experience_level
    WHEN 'very_experienced' THEN 'very_experienced'::experience_level
    WHEN 'beginner' THEN 'new'::experience_level  -- Map beginner to new
    ELSE 'new'::experience_level
  END;

  -- Map location type to enum value - FIX THE MAPPINGS
  ...
  INSERT INTO public.ai_programs (
    ...
    'user'::program_creator  -- FIX: Use 'user' instead of 'ai'
