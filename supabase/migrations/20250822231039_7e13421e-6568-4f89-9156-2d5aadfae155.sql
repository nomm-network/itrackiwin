...
-- 3) Temporary mapping from old table -> enum value (FIXED MAPPING)
with m as (
  select
    upf.id as upf_id,
    case el.slug
      when 'new' then 'new'::public.experience_level
      when 'returning' then 'returning'::public.experience_level
      when 'regular' then 'regular'::public.experience_level
      when 'advanced' then 'very_experienced'::public.experience_level
      else 'new'::public.experience_level
    end as new_enum
  from public.user_profile_fitness upf
  left join public.experience_levels el on el.id = upf.experience_level_id
)
...
-- 4) Make the new enum required and drop old FK/column
alter table public.user_profile_fitness
  alter column experience_level_enum set not null;

-- Drop FK to experience_levels if exists
do $$
begin
  if exists (
...
-- Try to migrate data if old params still exist
...
select distinct
  ...
  coalesce(p.start_intensity_low, 0.60), coalesce(p.start_intensity_high, 0.70),
  coalesce(p.warmup_set_count_min, 3), coalesce(p.warmup_set_count_max, 3),
  coalesce(p.main_rest_seconds_min, 90), coalesce(p.main_rest_seconds_max, 120),
  coalesce(p.weekly_progress_pct, 0.03), coalesce(p.allow_high_complexity, false)
from public.experience_levels el
left join public.experience_level_params p on p.experience_level_id = el.id
on conflict (experience_level) do nothing;

-- Insert defaults if no data was migrated
insert into public.experience_level_configs (
  experience_level, start_intensity_low, start_intensity_high,
  warmup_set_count_min, warmup_set_count_max,
  main_rest_seconds_min, main_rest_seconds_max,
...
-- 6) Drop old experience level translation & base tables
do $$
begin
...
-- 7) Rename enum column to final name
alter table public.user_profile_fitness
...
-- 1) Check if user_gym_memberships exists, create if not
...
values
  ...
  ('enum.experience_level.regular', 'ro', 'Antrenament regulat'),
  ('enum.experience_level.very_experienced', 'ro', 'Foarte experimentat')
