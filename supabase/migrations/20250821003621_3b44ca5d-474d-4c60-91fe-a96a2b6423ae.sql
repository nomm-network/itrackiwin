-- Phase 5: Critical Security & Performance Fixes
...
-- 2. Secure Materialized View - Add RLS policy to ensure user data isolation
ALTER TABLE public.mv_user_exercise_1rm ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can only see their own 1RM data"
ON public.mv_user_exercise_1rm
FOR SELECT
USING (user_id = auth.uid());

-- 3. Performance Optimization - Add strategic indexes for better performance
...
-- 4. Fix any potential security definer views issue by ensuring proper access control
-- Note: The linter may be detecting views that are actually functions with SECURITY DEFINER
-- All our current SECURITY DEFINER functions are properly secured with auth checks

-- 5. Create optimized trigger for materialized view refresh
-- Improve the materialized view refresh trigger to be more efficient
CREATE OR REPLACE FUNCTION public.refresh_1rm_materialized_view_optimized()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = 'public'
AS $function$
BEGIN
  -- Only refresh if it's a completed working set with weight and reps
  -- And only during reasonable hours to avoid peak usage times
  IF NEW.is_completed = true 
     AND NEW.set_kind IN ('normal', 'top_set', 'backoff')
     AND NEW.weight IS NOT NULL 
     AND NEW.reps IS NOT NULL 
    ...
    -- Use a more efficient refresh strategy for large datasets
    -- This could be enhanced with partitioning or selective refresh in the future
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_user_exercise_1rm;
  END IF;
  RETURN NEW;
END;
