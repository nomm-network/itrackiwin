-- Step 1: Check if table was already renamed, if not rename it
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'movement_translations') THEN
        ALTER TABLE movement_translations RENAME TO movements_translations;
    END IF;
END $$;

-- Step 2: Add sample translations for movements if missing
-- English translations for existing movements
...
ON CONFLICT (movement_id, language_code) DO NOTHING;

-- Romanian translations for movements
INSERT INTO movements_translations (movement_id, language_code, name, description)
SELECT 
  m.id,
  'ro' as language_code,
...
-- Step 3: Add sample translations for movement patterns if missing
...
ON CONFLICT (movement_pattern_id, language_code) DO NOTHING;
