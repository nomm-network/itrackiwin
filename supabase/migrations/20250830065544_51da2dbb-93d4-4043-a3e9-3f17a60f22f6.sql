-- Add 4 exercises with correct muscle slugs
DO $$
DECLARE
...
BEGIN
    -- Get muscle group IDs (using actual slugs from your DB)
    SELECT id INTO chest_muscle_id FROM muscles WHERE slug = 'mid_chest' LIMIT 1;
    SELECT id INTO shoulders_muscle_id FROM muscles WHERE slug = 'front_delts' LIMIT 1;
    ...
    IF chest_muscle_id IS NOT NULL AND machine_equipment_id IS NOT NULL THEN
        ...
        VALUES 
            ...
            ('flat-bench-press', COALESCE(bench_equipment_id, machine_equipment_id), 'press'::movement_pattern, chest_muscle_id, true, NOW())
    ...
    RAISE NOTICE 'Found chest muscle: %, shoulders muscle: %, machine equipment: %, dumbbell equipment: %', 
        chest_muscle_id, shoulders_muscle_id, machine_equipment_id, dumbbell_equipment_id;
