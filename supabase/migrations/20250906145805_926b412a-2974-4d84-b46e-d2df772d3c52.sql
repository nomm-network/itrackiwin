-- Step 2: Ambassador Program Scaffolding (Fixed)
...
CREATE TABLE IF NOT EXISTS public.ambassador_commission_agreements (
  ...
  ends_at TIMESTAMPTZ,  -- Will be set by trigger
  created_at TIMESTAMPTZ DEFAULT now(),
...
-- Trigger to set commission end date (5 years from start)
CREATE OR REPLACE FUNCTION public.set_commission_end_date()
RETURNS TRIGGER AS $$
BEGIN
  NEW.ends_at := NEW.starts_at + INTERVAL '5 years';
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER set_commission_end_date_trigger
  BEFORE INSERT OR UPDATE ON public.ambassador_commission_agreements
  FOR EACH ROW EXECUTE FUNCTION public.set_commission_end_date();
...
CREATE POLICY cities_sel ON public.cities FOR SELECT USING (true);
