...
-- First, let's create a few demo exercises to test the system
...
(
  ...
  'cable-lat-pulldown',
  'Cable Lat Pulldown',
  (SELECT id FROM equipment WHERE slug = 'cable-machine' LIMIT 1),
  true,
  null,
  true,
...
(
  ...
  'Dumbbell Chest Press', 
  (SELECT id FROM equipment WHERE slug = 'dumbbells' LIMIT 1),
  true,
  null,
  true,
  (SELECT id FROM muscle_groups WHERE slug = 'chest' LIMIT 1),
